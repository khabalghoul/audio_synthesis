...
class EquivarianceWidget:
    def __init__(self, viz):
        ...
        # self.s.bind(('localhost', CONSUMER_PORT))
        # self.s.listen()
        #print("Connected to server at", ('localhost', CONSUMER_PORT))
            ...
            if data:
                ...
                for message in messages:
                    if message:  # Ignore empty messages
                        ...
                        self.xlate.x = -float(data_dict['x'])
                        self.xlate.y = -float(data_dict['y'])
                        self.rotate.val = float(data_dict['angle'])
                        #self.rotate.val = self.rotate.val + float(data_dict['angle']) * 0.05
    ...
    def __call__(self, show=True):
        ...
        if show:
            ...
            with imgui_utils.item_width(viz.font_size * 8):
                _changed, (self.xlate.x, self.xlate.y) = imgui.input_float2('##xlate', -self.xlate.x, self.xlate.y,
                                                                            format='%.4f')
            ...
            with imgui_utils.item_width(-1 - viz.button_w - viz.spacing), imgui_utils.grayed_out(not self.xlate.anim):
                changed, speed = imgui.slider_float('##xlate_speed', self.xlate.speed, 0, 0.5, format='Speed %.5f',
                                                    power=5)
                if changed:
        ...
        if show:
            ...
            imgui.same_line(viz.label_w)
            with imgui_utils.item_width(viz.font_size * 8):
                _changed, self.rotate.val = imgui.input_float('##rotate', self.rotate.val % 1.0054, format='%.4f')
            imgui.same_line(viz.label_w + viz.font_size * 8 + viz.spacing)
            ...
            with imgui_utils.item_width(-1 - viz.button_w - viz.spacing), imgui_utils.grayed_out(not self.rotate.anim):
                changed, speed = imgui.slider_float('##rotate_speed', self.rotate.speed, -1, 1, format='Speed %.4f',
                                                    power=3)
                if changed:
