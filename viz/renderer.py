...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        if z_offset is not None:
            # viene como bytes → ndarray (512,)
            z_offset = np.frombuffer(z_offset, dtype=np.float32, count=G.z_dim)
            # suma a TODOS los seeds, manteniendo la forma (N,512)
            all_zs += z_offset[np.newaxis, :]
